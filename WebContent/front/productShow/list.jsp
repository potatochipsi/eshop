	...
	<body>
		...
		<div class="container">
			<c:forEach items="${productBeans}" var="item">
				<div class="product">
					<a href="${pageContext.request.contextPath}/front/productShow/productShowServlet?method=info&id=${item.id}">
						<div class="ima" style="background-image:url('${item.pic}');"></div>
					</a>
					<div>商品名称：<a href="productShowServlet?method=info&id=${item.id}">${item.name}</a></div>
		...
		${param.status == 2?"<div class='alert alert-success' role='alert'>订单已成功发出</div>":""}
	</body>
</html>
		...
		<div class="container-fluid">
				...
				<div class="col-md-10 col-md-offset-1">
					<table class="table">
						...
						<tr>
							...
							<td>
								<c:if test="${orderBean.payment_type == 0}">在线支付</c:if>
								<c:if test="${orderBean.payment_type == 1}">货到付款</c:if>
							</td>
							<td>
								<c:if test="${orderBean.status == 0}">未支付</c:if>
								<c:if test="${orderBean.status == 1}">已支付</c:if>
								<c:if test="${orderBean.status == 2}">未收货</c:if>
			...
			<div class="row-fluid">
				<div class="col-md-10 col-md-offset-2">
					<button class="btn" onclick="window.location.href='${pageContext.request.contextPath}/admin/userOrderServlet?method=delete&orderId=${orderBean.id}'">删除</button>
				</div>
			</div>
		</div>
	</body>

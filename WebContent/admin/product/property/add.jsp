...
<html>
				...
				if (id > 0) {
					...
					function(data) {
						if (data != null && data.length > 0) {
							var content = "<div class='col-sm-2' ><select name='productTypeId' class='form-control' onchange='chType(this)' id='type0'><option value='-1'>-- 请选择分类 --</option>";
	...
	<body>
		<div class="container">
			...
			<form id="myForm" role="form" action="productPropertyServlet?method=add" method="post">
				<div class="form-group col-md-12">
					<label id="label1" class="col-md-2 control-label"for="name">属性名： </label>
					<div class="col-md-10">
						<input class="form-control" name="name" type="text" id="name" placeholder="属性名" value="${productPropertyBean.name}"/>
					</div>
				</div>
			...
			<div class="form-group col-md-12">
				<div class="col-md-offset-2 col-md-10">
					<c:if test="${param.status.equals('1')}">
						<div class="alert alert-success"role="alert">添加属性成功</div>
					</c:if>
				</div>
			</div>	
		</div>
